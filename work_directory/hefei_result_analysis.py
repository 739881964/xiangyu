...
# @Author  : Yu xiang
# @File    : hefei_result_analysis.py
# @Software: PyCharm
# @Company : BEIJING INTENGINE


""" recognize rate testing script"""
...
from scripts.pandas_manual import PandasManual, ModifyExcelFormat, FixExcel
simplefilter(action='ignore', category=FutureWarning)


def get_slaver_board_log(base_path: str) -> str:
...
def get_mic_log(base_path: str) -> str:
    d_files = os.listdir(base_path)
    for file in d_files:
        if 'MIC' in file:
...
def get_lost_wav_start_time(all_start_time: list, data: 'pandas DataFrame 格式的数据') -> list:
    """获取未识别语音播放开始时间"""
...
def get_lost_wav(require_wav: list, data: 'pandas DataFrame 格式的数据') -> list:
    """
    获取未识别的音频
    data: 必须是DataFrame形式的数据，用pandas.DataFrame()转换
    """
    res = data['wav_name'].tolist()
    # lost_wav = [v for v in require_wav if v not in res]
...
def get_lost_command(all_wav: list, require_wav: list, need_command: list) -> list:
    """ 通过音频名称获取命令词 """
    commands = list()
...
def operation(log_res_len: list, test_count: list, need_wav: list, compare_command: list):
    """ 计算pass 和 fail """
    ...
    for i in range(len(test_count)):
        for j in range(len(log_res_len)):
            ...
            one_time = log_res_len[j][1:24]  # 时间所在的切片位置
            try:
...
def recognize_rate(data: dict, count_times: int, times: int, awake_command: list, time_list: list, all_command: list, count_command: list):
...
@run_time()
def main(result_path, base_path, n=1):
    """如果存在以前的excel结果，自动删除"""
    try:
    ...
    data = read_rstrip_data(mic_log)  # 串口打印日志路径
    ...
    (case_id, wav_name, start_time, expected_command,
    ...
    all_wav = get_lost_wav(require_wav, second_data)
    all_command_list = get_lost_command(all_wav, require_wav, need_command)
    lost_wav_command = {
        'wav_name': all_wav,
        'start_time': lost_time,
        'commands': all_command_list,
        'signel': ['lost'] * len(all_wav)
    }
    ...
    # 修改excel单元格显示问题
    # ModifyExcelFormat(EXCEL_PATH).func_open_py_xl_modify_excel(list(range(len(need_command) + 1)),
    #                                                            list(range(len(have_res) + 1))
    #                                                            )
    # FixExcel(EXCEL_PATH).modify_excel()
...
if __name__ == "__main__":
    lines = '---------------' * 2
    # 存放日志和测试结果的位置
    __file_path = 'path.yaml'
    yaml_content = read_yml(__file_path)
    __base_path = yaml_content['base_path']
