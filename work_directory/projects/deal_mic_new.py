...
def get_xls_list():
    ...
    if 'XIAOMEI' in filelist[0]:
        ...
        name = '语音分类垃圾桶'
    elif 'TONGYONGDUOLIAN' in filelist[0]:
        name = '通用多联灯控'
    ...
    if wavList:
        ...
        for i in range(len(allWord)):
            ...
            if 'XIAOMEI' in filename_1 or 'FUQIANG' in filename_1 or 'JIANFENG' in filename_1:
                ...
                if allWord[i] == '你好小美' or allWord[i] == '你好小白':
                    waketp += TP
                    wakeall += TOTAL
                else:
                    cmdtp += TP
                ...
                if allWord[i] == '小沐你好':
                    ...
                    cmdall += TOTAL
                    cmdfp += FP
            elif 'TONGYONGDUOLIAN' in filename_1:
                if allWord[i] == '你好小美' or allWord[i] == '你好小白' or allWord[i] == '小白你好':
                    waketp += TP
                    wakeall += TOTAL
                else:
                    cmdtp += TP
        ...
        elif 'DONGYONGDUOLIAN' in filename_1:
            work_book.save(r'D:\DONGYONGDUOLIAN' + name)
    else:
        pass
...
def send_mail(filelist, name):
    try:
        ...
        receivers = ['hswang@intenginetech.com', 'weisun@intenginetech.com',
                     'zjyan@intenginetech.com', 'zytang@intenginetech.com',
        ...
        smtp.sendmail(sender, receivers+cc_mail, str(message))  
        smtp.quit()
...
if __name__ == '__main__':
    ...
    if 'XIAOMEI' in log1:
        ...
        allWord = ['你好小美', '你好小白', '打开语音', '开启语音','语音开','关闭语音',
                    '关掉语音', '语音关', '关闭语音模式', '打开声音', '关闭声音', '增大音量', '音量大一点',
                    '声音大一点', '大声点', '大点声', '减小音量', '音量小一点', '声音小一点', '小声点', '小点声',
                    '打开灯光', '请开灯', '开灯', '开启电源', '关闭灯光', '请关灯', '关灯','关闭电源',
                    '冷白光', '白色光', '打开冷光', '自然光', '中性光', '打开自然光', '暖色光','黄色光',
                    '打开暖光', '调亮一点', '再亮一点', '亮一点', '灯太暗', '调暗一点','再暗一点',
                    '暗一点', '灯太亮', '最大亮度', '最高亮度', '调到最亮', '调到高档','中等亮度',
                    '调到中档', '最小亮度', '最低亮度', '调到最暗', '调到低档', '调到一档','调到二档',
                    '调到三档', '调到四档', '延时关灯', '定时关灯', '取消', '定时十分钟','定时二十分钟',
                    '定时半小时', '定时一小时', '定时两小时']
    ...
    elif 'TONGYONGDUOLIAN' in log1:
        allWord = ['小白你好', '你好小白', '你好小美', '打开壁灯',
