...
# wav_path = 'D:/AI/seven/'
wav_path = r'\\192.168.1.12\hftest\hfwav\tongyongduolian2'
...
def get_wave_files(path):
    wav_file = []
    for (dir_path, dir_names, filenames) in os.walk(path):  # 访问文件夹下的所有文件
        # os.walk() 方法用于通过在目录树种游走输出在目录中的文件名，向上或者向下
        for filename in filenames:
            # if filename.endswith('.wav') or filename.endswith('.WAV'):
            if filename.endswith('.wav'):
                if 'SPEAK' not in filename:
                    if 'TJ0030030@TJ0030030B03_0065.wav' in filename:
                        continue
                    # endswith() 方法用于判断字符串是否以指定后缀结尾，如果以指定后缀结尾返回True，否则返回False
                    else:
                        filename_path = os.sep.join([dir_path, filename])  # 定义文件路径(连)
                        # print(os.stat(filename_path).st_size)
                        # if os.stat(filename_path).st_size < 32000:#st_size文件的大小，以位为单位
                        #    continue
                        wav_file.append(filename_path)  # 加载文件
    return wav_file


wav_files = get_wave_files(wav_path)  # 获取文件名列表


# 读取wav文件对应的label
def get_wav_label(data):

    _labels = []
    new_wav_files = []
    for wav_file in data:
        wav_id = os.path.basename(wav_file).split('.')[0]
        _labels.append(wav_title)  # 每条语音对应的标签
        new_wav_files.append(wav_file)

    return new_wav_files, _labels  # 返回标签和对应的文件


wav_files, labels = get_wav_label(wav_files)  # 得到标签和对应的语音文件
