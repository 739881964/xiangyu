...
class select(object):
    ...
    def get_ticket_info():
        ...
        print(
            ...
            f"到达站：{TickerConfig.TO_STATION}\n"
            f"车次: {','.join(TickerConfig.STATION_TRAINS) or '所有车次'}\n"
            f"乘车日期：{','.join(TickerConfig.STATION_DATES)}\n"
            f"坐席：{','.join(TickerConfig.SET_TYPE)}\n"
            f"是否有票优先提交：{TickerConfig.IS_MORE_TICKET}\n"
            f"乘车人：{TickerConfig.TICKET_PEOPLES}\n"
            f"刷新间隔: 随机(1-3S)\n"
            f"僵尸票关小黑屋时长: {TickerConfig.TICKET_BLACK_LIST_TIME}\n"
            f"下单接口: {TickerConfig.ORDER_TYPE}\n"
            f"下单模式: {TickerConfig.ORDER_MODEL}\n"
            f"预售踩点时间:{TickerConfig.OPEN_TIME}"
        )
        print(u"*" * 50)
        ...
        while True:
            try:
                ...
                if queryResult.get("status", False):
                    ...
                    print(f"正在第{num}次查询 随机停留时长：{random_time} 乘车日期: {','.join(TickerConfig.STATION_DATES)} "
                          f"车次：{','.join(TickerConfig.STATION_TRAINS) or '所有车次'} "
                          f"下单无票{nateMsg} "
                          f"耗时：{(datetime.datetime.now() - now).microseconds / 1000}ms"
                          )
